...
//	import mx.utils.URLUtil;
	...
	public class FlashSocket extends EventDispatcher implements IWebSocketWrapper
	{
		...
		public function FlashSocket( domain:String,urlparams:String = '', protocol:String=null, proxyHost:String = null, proxyPort:int = 0, headers:String = null)
		{
			...
			r.url = "http://" + domain + "/socket.io/1/?time=" + new Date().getTime() +'&' + urlparams;
			...
			ul.addEventListener(IOErrorEvent.NETWORK_ERROR , onDiscoverError);
			ul.addEventListener(IOErrorEvent.VERIFY_ERROR , onDiscoverError);
			//*/
		}
			...
			if ( event is HTTPStatusEvent ){
				if ( (event as HTTPStatusEvent).status != 200){
					//we were unsuccessful in connecting to server for discovery
					 fe = new FlashSocketEvent(FlashSocketEvent.CONNECT_ERROR);
					dispatchEvent(fe);
				}
			}else{
				 fe = new FlashSocketEvent(FlashSocketEvent.CONNECT_ERROR);
		...
		protected function loadDefaultPolicyFile(wsUrl:String):void {
			var policyUrl:String = "xmlsocket://" + URLUtil.getServerName(wsUrl) + ":10843";//use 10843
			...
			*/
			...
			switch ( dm.type ){
				...
				case '2':
					...
					break;
				case '3':
					var fem:FlashSocketEvent = new FlashSocketEvent(FlashSocketEvent.MSG_STR);
					fem.data = dm.msg;
					dispatchEvent(fem);
					break;
				case '4':
		...
		//server端 
		//产生一个事件
		public function emit(event:String,msg:Object=null):void{
